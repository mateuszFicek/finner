...
import '../../domain/use_cases/check_user_status.dart';
  ...
  AccountBloc(
    ...
    on<AccountEvent>((event, emit) async {
      ...
      if (event is _SignUp) {
        ...
        r.fold(
          (e) => emit(AccountState.accountError(e.message)),
          (t) => emit(const AccountState.accountCreated()),
        );
      }
      ...
      if (event is _CheckUserStatus) {
        emit(const AccountState.accountInformationLoading());
        final r = await _checkUserStatus();
        r.fold(
          (e) => emit(AccountState.accountError(e.message)),
          (t) {
            t != null
                ? emit(const AccountState.signedIn())
                : emit(const AccountState.accountError("No user."));
          },
        );
      }
    });
